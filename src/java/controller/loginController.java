...
public class loginController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Account> accounts = dao.gettAll();
        ...
        for (Account account : accounts) {
            if (user.equals(account.getUsername()) && pass.equals(account.getPassword())) {
                if (account.getPriority() == 1) {
                    ...
                    return;
                } else if (account.getPriority() == 2) {
                    Account acc = new Account(user, pass, 2);
                    session.setAttribute("account", acc);
                    response.sendRedirect("home.jsp");
                    return;
                }
        ...
        request.setAttribute("incorrectAccount", "Username or Password incorrect");
        request.getRequestDispatcher("login.jsp").forward(request, response);
